...
use crate::{instruction::BuybakStatistics, instruction::BuybakPortfolio, instruction::UserProfile, instruction::UserPortfolio};
...
pub fn unpack_buybak_portfolio_from_slice(src: &[u8]) -> Result<(bool, BTreeMap<String, BuybakPortfolio>), Box<dyn Error>> {
    ...
    if data_len == 0 {
        Ok((is_initialized, BTreeMap::<String, BuybakPortfolio>::new()))
    } else {
        let data_dser = BTreeMap::<String, BuybakPortfolio>::try_from_slice(&data_src[0..data_len]).unwrap();
...
/// Unpacks the data from slice and return the initialized flag and data content
#[allow(clippy::ptr_offset_with_cast)]
pub fn unpack_buybak_statistics_from_slice(src: &[u8]) -> Result<(bool, BTreeMap<String, BuybakStatistics>), Box<dyn Error>> {
    let src = array_ref![src, 0, ACCOUNT_STATE_SPACE];
    // Setup pointers to key areas of account state data
    let (is_initialized_src, data_len_src, data_src) =
        array_refs![src, INITIALIZED_BYTES, BTREE_LENGTH, BTREE_STORAGE];
    ...
    // If emptry, create a default
    if data_len == 0 {
        Ok((is_initialized, BTreeMap::<String, BuybakStatistics>::new()))
    } else {
        let data_dser = BTreeMap::<String, BuybakStatistics>::try_from_slice(&data_src[0..data_len]).unwrap();
        Ok((is_initialized, data_dser))
    }
}
...
/// Packs the initialized flag and data content into destination slice
#[allow(clippy::ptr_offset_with_cast)]
pub fn pack_buybak_statistics_into_slice(
    is_initialized: bool,
